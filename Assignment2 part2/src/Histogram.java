...
public class Histogram extends JPanel {
	...
	private void drawBins(Graphics g) { 
	  ...
	  this.binWidth =((getWidth()- LEFT_MARGIN - RIGHT_MARGIN)/bins.length);// this the value i
	
	  double binheight = (getHeight()-TOP_MARGIN-60)/max; // binHeight is the ratio that is used in
	  // scaleY method to compute the graphical equivalent .
	  
	  int i = 0; // initializing a variable to run through the indexes of the bins[] array.
	  
	 for ( int n : bins){
	      int x = LEFT_MARGIN + (i*binWidth);// this variable x will initialize the each value of
	      // of the bin array.
	      ...
	      int y = (getHeight()- (int)heights - TOP_MARGIN);// from this variable we are obtaining the 
	      // y value of the bin.
	      
	      int y_height = getHeight()-1-BOTTOM_MARGIN-y; // from this y_hight we can the height of the
	      // bin from the point y.
	      
	      g.fillRect(x, y, binWidth-1, y_height );// drawing the bin using
	      // the fillRect method from the graphics.
	...
	//String label;
	 double binheight = (getHeight()-TOP_MARGIN-60)/max;// this binheight is the ratio that used to calculate the
	 // graphical equivalent of the value from the array.
	for ( int i : bins){
	 ...
	 * drawing the rectangle. This method is only used inside of the histogram glass, this is 
	 * why this method is made as private method.
	 */
	private double scaleY(int value, double height){
		 ...
		 return result+22; // returning the value, here i am adding 25 just to make the small bins
		 // noticeable in our picture.
	}
	/*
	 * This method is used to get the maximum value for the bins array that is containing the elements
	 * is each bin and returns the maximum value. This method is only used inside of the class
